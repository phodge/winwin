...
def ui_new_session(name: str) -> None:
    ...
    if platform == Platforms.TERMINAL_TMUX.value:
        begin_session_terminal_tmux(cfg, name, remote_host, want_2nd_terminal)
    else:
        raise Exception("TODO: spawn session using platform %r" % (platform, ))  # noqa


def begin_session_terminal_tmux(cfg: Config, name, remote_host, want_2nd_terminal):
    if cfg.terminal_is_alacritty:
        from winwin.alacritty import open_alacritty_gui
    else:
    ...
    # first screen
    cmd = get_self_cmd(['enter-main-session', name])
    if cfg.terminal_is_alacritty:
        open_alacritty_gui(cfg, title1, cmd, 'main')
    else:
        open_terminal_gui(title1, cmd)
    ...
    if want_2nd_terminal:
        cmd2 = get_self_cmd(['enter-second-session', name])
        if cfg.terminal_is_alacritty:
            open_alacritty_gui(cfg, title2, cmd2, 'secondary')
        else:
            open_terminal_gui(title2, cmd2)
